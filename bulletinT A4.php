...
<head>
    ...
    <header class="premium-header no-print">
        <div style="display: flex; gap: 15px;">
            ...
            <a href="T A4.html" class="nav-link back">
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $noteClasseAll1 = (float) htmlspecialchars($_POST['noteClasseAll1']);
    $noteClasseAll2 = (float) htmlspecialchars($_POST['noteClasseAll2']);
    $noteCompAll = (float) htmlspecialchars($_POST['noteCompAll']);
    $profCompAll = htmlspecialchars($_POST['profCompAll']);
    ...
    $coefEsp = 2; // Coefficient fixe pour LV2 si saisie
    ...
    // Calcul du total des coefficients (Base 18 pour les matières obligatoires restantes)
    $totalcoef = 18 + (float)$coef + (float)$coefE;
    
    // Points pour les matières obligatoires
    $sommeMoyennes = (
        ((($noteCompFrancaise + $moyenneFrancaise) / 2) * 4) +
        ((($noteCompAnglais + $moyenneAnglais) / 2) * 3) +
        ((($noteCompHistoireGeo + $moyenneHistoireGeo) / 2) * 3) +
        ((($moyennePhilo + $noteCompPhilo) / 2) * 4) +
        ((($noteCompMath + $moyenneMath) / 2) * 2) +
        ((($noteCompSVT + $moyenneSVT) / 2) * 1) +
...
?>
    ...
    <td colspan="11" style="text-align: center; font-weight: bold; border-top: 1px solid black; padding: 5px;">MATIERES LITTERAIRES</td>
            ...
            <tbody>
                <tr>
                    ...
                    <td>3</td>
                    <td><?php echo number_format(floor((floatval($noteCompFrancaise) + floatval($moyenneFrancaise)) / 2 * 3 * 100) / 100, 2, ',', ' '); ?></td>
 ...
 <!-- ECM -->
            ...
            <tr>
                ...
                <td>4</td>
                <td><?php echo number_format(floor((floatval($noteCompPhilo) + floatval($moyennePhilo)) / 2 * 4 * 100) / 100, 2, ',', ' '); ?></td>
                <td><?php echo $observationPhilo; ?></td>
               <td><?php echo $profCompPhilo; ?></td>
                <td></td>
            </tr>

            <?php if ($moyenneAll !== 0 || $noteCompAll !== 0): ?>
            <tr>
                <td>Allemand</td>
                <td><?php echo (float)$noteClasseAll1 ?: ''; ?></td>
                <td><?php echo (float)$noteClasseAll2 ?: ''; ?></td>
                <td><?php echo number_format($moyenneAll, 2, ',', ' '); ?></td>
                <td><?php echo (float)$noteCompAll ?: ''; ?></td>
                <td><?php echo number_format(($moyenneAll + $noteCompAll) / 2, 2, ',', ' '); ?></td>
                <td>2</td>
                <td><?php echo number_format(floor((floatval($noteCompAll) + floatval($moyenneAll)) / 2 * 2 * 100) / 100, 2, ',', ' '); ?></td>
            ...
            <tr>
                ...
                <td>2</td>
                <td><?php echo ($moyenneEsp !== null && $noteCompEsp !== null) ? number_format(floor((floatval($noteCompEsp) + floatval($moyenneEsp)) / 2 * 2 * 100) / 100, 2, ',', ' ') : ''; ?></td>
            ...
            <?php endif; ?>
                ...
                <tr>
                    ...
                    <td>3</td>
                    <td><?php echo number_format(floor((floatval($noteCompAnglais) + floatval($moyenneAnglais)) / 2 * 3 * 100) / 100, 2, ',', ' '); ?></td>
            ...
            <tr>
                ...
                <td>3</td>
                <td><?php echo number_format(floor((floatval($noteCompHistoireGeo) + floatval($moyenneHistoireGeo)) / 2 * 3 * 100) / 100, 2, ',', ' '); ?></td>
            ...
            <tr>
                ...
                <td>2</td>
                <td><?php echo number_format(floor((floatval($noteCompMath) + floatval($moyenneMath)) / 2 * 2 * 100) / 100, 2, ',', ' '); ?></td>
            ...
            <tr>
                ...
                <td>1</td>
                <td><?php echo number_format(floor((floatval($noteCompSVT) + floatval($moyenneSVT)) / 2 * 1 * 100) / 100, 2, ',', ' '); ?></td>
            ...
            <tr>
                ...
                <td>1</td>
                <td><?php echo number_format(floor((floatval($noteCompPhysique) + floatval($moyennePhysique)) / 2 * 1 * 100) / 100, 2, ',', ' '); ?></td>
            ...
            <tr>
                <td>Musique/EM/Ewe/Kabye/Couture/<br>Espagnol/Dessin</td>
...
<div>
 ...
 <script>
   function downloadPDF() {
        ...
        var filename = (nomEleve.replace(/\s+/g, '_') + '_' + classe.replace(/\s+/g, '_') + '.pdf').toLowerCase();

        var element = document.querySelector('.container');
        if (!element) {
            alert('Erreur : contenu du bulletin introuvable.');
            return;
        }
        ...
        html2pdf().set(opt).from(element).save().then(function() {
            if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-file-pdf"></i> Télécharger PDF'; }
        });
    }
</script>
